...
const keyworkerApiFactory = client => {
  ...
  const getAssignedOffenders = (context, staffId, prisonId) =>
    get(context, `key-worker/${staffId}/prison/${prisonId}/offenders`)

  const getKeyworkerByStaffIdAndPrisonId = (context, staffId, prisonId) =>
    get(context, `key-worker/${staffId}/prison/${prisonId}`)
