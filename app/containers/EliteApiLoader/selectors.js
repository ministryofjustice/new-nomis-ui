...
const selectBookingResultStatus = ({ query, pagination, sortOrder }) =>
  createSelector(selectEliteApi(), eliteApiState =>
    calcBookingResultStatus(eliteApiState, { query, pagination, sortOrder })
  )
...
const selectBookingResults = ({ query, pagination, sortOrder }) =>
  createSelector(selectEliteApi(), eliteApi => calcBookingResults(eliteApi, { query, pagination, sortOrder }))

const selectImages = () => createSelector(selectEliteApi(), eliteApi => eliteApi.get('Images'))

const selectImageId = () => (_, props) => props.imageId

const selectImage = () => createSelector(selectImages(), selectImageId(), (images, id) => images.get(id))

const selectOfficers = () => createSelector(selectEliteApi(), eliteApi => eliteApi.get('Officers'))

const selectOfficerKey = () => (_, props) => props.officerKey

const selectOfficer = () =>
  createSelector(selectOfficers(), selectOfficerKey(), (officers, id) => {
    officers.get(id)
  })

const selectOfficerStatus = () =>
  createSelector(selectOfficer(), officer => {
    if (officer) return officer.get('Status').toJS()
    return { Type: 'NOT STARTED' }
  })

const selectBookingDetails = () => createSelector(selectEliteApi(), eliteApi => eliteApi.getIn(['Bookings', 'Details']))

const selectAlertTypes = () => createSelector(selectEliteApi(), eliteApi => eliteApi.get('AlertTypes'))

const selectAlertTypeId = () => (_, props) => props.alertType

const selectAlertTypeObj = () =>
  createSelector(selectAlertTypes(), selectAlertTypeId(), (alertTypes, alertTypeId) => alertTypes.get(alertTypeId))

const selectAlertType = () =>
  createSelector(selectAlertTypeObj(), alertTypeObj => {
...
const selectAlertCodeObj = () =>
  createSelector(selectAlertTypeObj(), selectAlertCodeId(), (alertTypeObj, alertCode) => {
    if (!alertTypeObj) return undefined
    return alertTypeObj.getIn(['Codes', alertCode])
  })

const selectAlertTypeCodeStatus = () =>
  createSelector(selectAlertCodeObj(), alertCodeObj => {
...
const selectCaseNoteTypes = () => createSelector(selectEliteApi(), eliteApi => eliteApi.getIn(['CaseNoteTypes']))

const selectCaseNoteTypesSelect = () =>
  createSelector(selectEliteApi(), eliteApi => eliteApi.getIn(['CaseNoteTypesSelect', 'TypeList']))

const selectCaseNoteSourceId = () => (_, props) => props.source

const selectCaseNoteSourceObj = () =>
  createSelector(selectCaseNoteTypes(), selectCaseNoteSourceId(), (caseNoteTypes, source) => caseNoteTypes.get(source))

const selectCaseNoteSourceStatus = () =>
  createSelector(selectCaseNoteSourceObj(), caseNoteSource =>
    caseNoteSource ? caseNoteSource.getIn(['Status', 'Type']) : undefined
  )

const selectCaseNoteSource = () =>
  createSelector(selectCaseNoteSourceObj(), caseNoteSource => (caseNoteSource ? caseNoteSource.get('Data') : undefined))

const selectCaseNoteTypeAndSubtype = () => (_, props) => ({ type: props.type, subType: props.subType })

const selectCaseNoteTypeDetails = () =>
  createSelector(selectCaseNoteSource(), selectCaseNoteTypeAndSubtype(), (caseNoteSource, { type, subType }) => {
...
const selectLoadingBookingDetailsStatus = () =>
  createSelector(selectEliteApi(), state => state.getIn(['Bookings', 'Details', 'LoadingStatus']))
