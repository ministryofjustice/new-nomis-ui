...
const AmendCaseNote = ({ handleSubmit, error, submitting, extendSession, goBackToBookingDetails }) => {
  if (error) {
    window.scrollTo(0, 0)
  }
    ...
    onSubmit: createFormAction(
      formData => ({
        ...
        payload: {
          ...
          caseNoteListReferrer: props.location && props.location.state && props.location.state.from,
        },
      }),
...
export const validate = form => {
  ...
  if (!amendmentText) {
    ...
    error.amendmentText = `Maximum length should not exceed 30000 characters (over by ${amendmentText.length -
      30000} characters)`
  }
  return error
}
