...
describe('controller tests', () => {
  describe('alerts', () => {
    ...
    it('Should reject " " as an offender number', () =>
      request
        .get('/app/bookings/ /alerts')
        .expect(400)
    );

    it('Should reject "x" as an offender number', () =>
      request
        .get('/app/bookings/x/alerts')
        .expect(400)
    );

    it('Should accept "X1" as an offender number', () =>
      request
    ...
    it('Should invoke elite2Api.get, combining the query parameters correctly', () =>
      request
        ...
        .expect(200)
        .then(() => {
          expect(elite2Api.get.mock.calls[0][1])
            .toBe("/api/bookings/1/alerts?query=dateCreated:gteq:DATE'2011-01-01',and:dateCreated:lteq:DATE'2020-12-31',and:alertType:in:'R'")
        })
    );

    it('Should reject an invalid "to" value', () =>
      request
        .get('/app/bookings/XXX/alerts?to=2011-01-011')
        .expect(400)
    );

    it('Should reject an invalid "from" value', () =>
      request
        .get('/app/bookings/XXX/alerts?to=201-01-01')
        .expect(400)
    );

    it('Should reject an invalid "alertType" value', () =>
      request
        .get('/app/bookings/XXX/alerts?alertType=999')
        .expect(400)
    );
  })
});